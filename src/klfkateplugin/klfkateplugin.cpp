...
static HiAttBlock make_normal_text_att(int start, int length)
{
  HiAtt *a = new HiAtt;
...
static void ensure_attr_fill_order_start(KTextEditor::Document *doc, int line, HiAttList * attlist)
{
  int llen = doc->lineLength(line);
  if (attlist->isEmpty()) {
    if (llen == 0)
      return;
...
/* // KDE already defines this
  inline QDebug operator<<(QDebug dbg, const Cur& c)
  {
  return dbg << "[l."<<c.line()<<" "<<c.column()<<"]";
  }
*/
...
static MathModeBoundary boundary_info(KTextEditor::Document * doc, const HiAttElem& attmin, const HiAttElem& attmax)
{
...
static MathModeBoundary
/* */ find_mathmode_transition(KTextEditor::Document * doc, KTextEditor::HighlightInterface * hiface,
			       Cur pos, bool forward, bool wantTrueBoundary = true)
{
  ...
  do {
    klfDbg("line iteration: line="<<line<<":\n\t> "<<qPrintable(doc->line(line))) ;

    HiAttList attlist = hiface->lineAttributes(line);
    ...
    ensure_attr_fill_order_start(doc, line, &attlist);
	...
	MathModeBoundary m = boundary_info(doc, attmin, attmax);
	...
	MathModeBoundary m2;
	m2 = find_mathmode_transition(doc, hiface, step_cur(doc, m.cur, forward), forward,
	...
	Cur cc = step_cur(doc, m2.cur, forward);
	MathModeBoundary mfinal = find_mathmode_transition(doc, hiface, cc, forward, wantTrueBoundary);
...
void KLFKtePluginView::slotReparseCurrentContext_KatePart(KTextEditor::Document * doc)
{
  ...
  headm = find_mathmode_transition(doc, hiface, curPos, false /*backwards*/);
  ...
  tailm = find_mathmode_transition(doc, hiface, curPos, true /*forward*/);
  if (!tailm.cur.isValid()) {
    klfDbg("failed to parse tail mathmode boundary: invalid tailm returned.") ;
    return;
  }

  KLF_ASSERT_CONDITION(!tailm.isstart, "!!: tailcur is not math-mode end ?!", return; ) ;

  // calculate mathmode region, get string
  QString latexmath = doc->text(KTextEditor::Range(headm.cur, tailm.cur));

  klfDbg("Boundaries: "<<headm<<" --- "<<tailm<<"\n\tGot latex math: "<<latexmath) ;

  QMessageBox::information(NULL, "latex", latexmath);
